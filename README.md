...
### 5. 导出功能
- 支持格式：HTML、PDF、Markdown、DOCX
- 图片自动嵌入为 Base64（无需网络依赖）
- **主题样式**：学术风格、现代风格、深色模式、紧凑模式
- **数学公式**：完整支持 `\$...\$`、`\$\$...\$\$`、`\\(...\\)`、`\\[...\\]`、`\begin{}...\end{}` 五种 LaTeX 定界符，KaTeX 渲染
- **公式健壮性**：行内公式允许包含 `)` 和换行（惰性匹配），覆盖 `\triangleq`/`\mathcal`/`\mathbf`/`\boldsymbol`/`\operatorname`/`\begin{aligned}` 等复杂符号和环境
- **HTML 规范**：自动修复 Markdown 转换产生的 `<p><div>` 非法嵌套
- **图片定位**：上下文感知的图片位置映射，按原文位置比例插入译文对应段落
- **DOCX 增强**：`<img>`→`![]()` 预转换防丢失、列表/引用行内格式化、公式斜体居中显示
- **MD 增强**：HTML 标签剥离（仅保留 `<br>`/`<hr>`）、图片 `![]()` 标准格式
- **PDF 增强**：封面页、水印（位置/平铺模式）、目录生成、表格自适应宽度
- **HTML 增强**：最大宽度 900px 居中排版、`word-break` 防表格溢出
