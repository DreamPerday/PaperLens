...
STATIC_DIR = Path(storage_service.base) / "static" / "images"
...
def export_document(project_id: str, doc_id: str, body: dict = Body(...)):
    ...
    parser = DocumentParser(file_path)
    original_text = parser.extract_text() if include_original else ""

    translation = storage_service.get_translation(project_id, doc_id)
    translated_text = translation["content"] if translation and include_translation else ""
    ...
    if fmt == "md":
        ...
        content = "\n\n".join(md_lines)
        return {"data": {"content": content, "mime": "text/markdown; charset=utf-8", "filename": f"{doc.get('original_name','export')}.md"}}

    if fmt == "pdf":
        html_parts = ["<html><head><meta charset='utf-8'><style>",
                      "body{font-family:'SimSun',serif;max-width:900px;margin:0 auto;padding:2rem;line-height:1.8}",
                      "h1{font-size:1.5em;border-bottom:1px solid #ddd;padding-bottom:.5em;margin-top:1.5em}",
                      "h2{font-size:1.2em;margin-top:1em}h3{font-size:1.05em}",
                      ...
                      "</style></head><body>"]
        if original_text:
            html_parts.append("<h1>Original</h1>")
            html_parts.append(original_text.replace("\n", "<br>"))
        if translated_text:
            html_parts.append("<h1>Translation</h1>")
            html_parts.append(translated_text.replace("\n", "<br>"))
        html_parts.append("</body></html>")
        content = "".join(html_parts)
        if embed_images:
            content = _embed_images_in_html(content)
        return {"data": {"content": content, "mime": "text/html; charset=utf-8", "filename": f"{doc.get('original_name','export')}.html"}}

    # HTML export
    html_parts = ["<!DOCTYPE html><html lang='zh-CN'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>",
                  doc.get('original_name', 'Translation'),
                  "</title><style>",
                  "body{font-family:'Segoe UI',system-ui,-apple-system,sans-serif;max-width:960px;margin:0 auto;padding:2rem;line-height:1.75;color:#1a1a2e}",
                  "h1{font-size:1.5rem;border-bottom:2px solid #e2e8f0;padding-bottom:.5rem;margin-top:2rem}",
                  "h2{font-size:1.25rem;margin-top:1.5rem}h3{font-size:1.1rem;margin-top:1.25rem}",
                  "p{margin:.75rem 0}img{max-width:100%;height:auto;display:block;margin:1rem auto}",
                  "pre{background:#f1f5f9;padding:1rem;border-radius:8px;overflow-x:auto}",
                  "code{background:#f1f5f9;padding:.15rem .3rem;border-radius:4px;font-size:.9em}",
                  "table{border-collapse:collapse;width:100%;margin:1rem 0}",
                  "th,td{border:1px solid #e2e8f0;padding:.5rem;text-align:left}",
                  "th{background:#f8fafc;font-weight:600}",
                  ".math-display{text-align:center;padding:.5rem 0}",
                  "</style></head><body>"]
    if original_text:
        html_parts.append("<h1>Original</h1>")
        html_parts.append(original_text)
    if translated_text:
        html_parts.append("<h1>Translation</h1>")
        html_parts.append(translated_text)
    html_parts.append("</body></html>")
    content = "".join(html_parts)
    if embed_images:
        content = _embed_images_in_html(content)
